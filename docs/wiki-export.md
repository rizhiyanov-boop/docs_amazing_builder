...
`clientResponseCode` и `responseCode` добавляются внутрь соответствующих ячеек как `{code:json}` macro, если заполнены.
...
- `{` заменяется на `&#123;`;
- `}` заменяется на `&#125;`;
- переносы строк внутри ячейки заменяются на `<br/>`;
