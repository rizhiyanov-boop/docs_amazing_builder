  ...
  return (
    <header ref={topbarRef} className="topbar">
      <div className="topbar-context">
        <div className="brand">
          <span className="logo" aria-hidden>API</span>
          <div><h1>Doc Builder</h1></div>
        </div>
      </div>
      ...
      <div className="actions">
        <div className="actions-main topbar-cluster" aria-label="Импорт и экспорт">
          <div className="topbar-export-group">
            <span className="topbar-group-label">МЕТОД</span>
            ...
            />
            <button className="ghost topbar-action topbar-format-btn" onClick={onExportProjectJson} disabled={!canExport} title={canExport ? `Скачать JSON. ${exportTitle}` : exportTitle} aria-label="Скачать JSON">
              <span className="ui-icon export-format-icon" data-format="JSON" aria-hidden>{renderUiIcon('download')}</span>
            </button>
            <button className="ghost topbar-action topbar-format-btn" onClick={onExportMockServiceJson} disabled={!canExport} title={canExport ? `Скачать mock-service JSON. ${exportTitle}` : exportTitle} aria-label="Скачать mock-service JSON">
              <span className="ui-icon export-format-icon" data-format="MOCK" aria-hidden>{renderUiIcon('download')}</span>
            </button>
            <button className="primary topbar-action topbar-format-btn" data-onboarding-anchor="export-docs" onClick={onOpenHtmlPreview} disabled={!canExport} title={canExport ? `Открыть HTML-предпросмотр. ${exportTitle}` : exportTitle} aria-label="Открыть HTML-предпросмотр">
              <span className="ui-icon export-format-icon" data-format="HTML" aria-hidden>{renderUiIcon('download')}</span>
            </button>
            <button className="topbar-action topbar-format-btn" onClick={onOpenWikiPreview} disabled={!canExport} title={canExport ? `Открыть Wiki-предпросмотр. ${exportTitle}` : exportTitle} aria-label="Открыть Wiki-предпросмотр">
              <span className="ui-icon export-format-icon" data-format="WIKI" aria-hidden>{renderUiIcon('download')}</span>
            </button>
          </div>

          <div className="topbar-export-group">
            <span className="topbar-group-label">ПРОЕКТ</span>
            <button className="ghost small" type="button" onClick={onExportFullProjectHtml} disabled={!canExport} title={canExport ? 'Скачать полный проект в HTML' : exportTitle}>
              Проект HTML
            </button>
            <button className="ghost small" type="button" onClick={onExportFullProjectWiki} disabled={!canExport} title={canExport ? 'Скачать полный проект в Wiki' : exportTitle}>
              Проект Wiki
            </button>
          </div>
        </div>
        ...
        <div className="actions-side">
          ...
          <div className="topbar-cluster topbar-system-cluster">
            {authLoading ? (
              <span className="badge topbar-auth-pending"><span className="ai-loader ai-loader-inline" aria-hidden="true" /><span>Проверка входа...</span></span>
            ) : authUserLogin ? (
              <details className="topbar-user-menu">
                <summary className="topbar-user-pill" title={authUserLogin}>
                ...
                <div className="topbar-user-menu-popover">
                  <button type="button" className="ghost small" onClick={onLogout} disabled={isLogoutBusy} aria-busy={isLogoutBusy}>
                    {isLogoutBusy ? (<><span className="ai-loader ai-loader-inline" aria-hidden="true" /><span>Выход...</span></>) : 'Выйти'}
                  </button>
                </div>
              </details>
            ) : (
              <>
                <button type="button" className="ghost small" onClick={onOpenLogin}>Войти</button>
                <button type="button" className="ghost small" onClick={onOpenRegister}>Регистрация</button>
              </>
            )}

            {isOnboardingHeaderAvailable && (
              <button type="button" className={`onboarding-nav-toggle ${isOnboardingNavVisible ? 'active' : ''}`} onClick={onToggleOnboardingHeaderNavigation} aria-label={isOnboardingNavVisible ? 'Скрыть панель навигации' : 'Показать панель навигации'} title={isOnboardingNavVisible ? 'Скрыть панель навигации' : 'Показать панель навигации'}>
                <span className="ui-icon onboarding-nav-icon" aria-hidden>{renderUiIcon('onboarding_nav')}</span>
              </button>
            )}
            <button type="button" className={`onboarding-nav-toggle ${!isSidebarHidden ? 'active' : ''}`} onClick={onToggleSidebar} aria-label={isSidebarHidden ? 'Показать боковую панель' : 'Скрыть боковую панель'} title={isSidebarHidden ? 'Показать боковую панель' : 'Скрыть боковую панель'}>
              <span className="ui-icon onboarding-nav-icon" aria-hidden>{renderUiIcon('sidebar_panel')}</span>
            </button>
            <button type="button" className="theme-mermaid-toggle" onClick={onToggleTheme} aria-label={theme === 'dark' ? 'Включить светлую тему' : 'Включить ночную тему'} title={theme === 'dark' ? 'Включить светлую тему' : 'Включить ночную тему'}>
      ...
      {isOnboardingHeaderAvailable && isOnboardingNavVisible && (
        <section className="onboarding-stepbar collapsed in-topbar" data-onboarding-anchor="prepare-source" aria-live="polite" aria-label="Пошаговая навигация">
          <div className="onboarding-stepbar-track" role="list" aria-label="Прогресс шагов">
            {onboardingSteps.map((step) => {
              ...
              return (
                <button key={step.id} type="button" role="listitem" className={`onboarding-step-chip ${isDone ? 'done' : ''} ${isCurrent ? 'current' : ''} ${access.allowed ? 'available' : 'blocked'}`} aria-current={isCurrent ? 'step' : undefined} aria-disabled={!access.allowed} disabled={!access.allowed} title={access.allowed ? step.description : access.reason} onClick={() => onJumpToOnboardingStep(step.id)}>
                  <span aria-hidden="true">{isDone ? '[x] ' : '[ ] '}</span>{step.title}
                </button>
              );
            })}
          </div>
          ...
          <div className="onboarding-stepbar-actions">
            <button type="button" className="small onboarding-stepbar-icon-btn primary" aria-label={onboardingPrimaryActionLabel} title={onboardingPrimaryActionLabel} onClick={onPrimaryOnboardingAction}>
