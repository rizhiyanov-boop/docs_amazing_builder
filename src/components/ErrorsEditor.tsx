...
import type { ErrorRow, ErrorsSection, ParsedRow, ParsedSection, ValidationRuleRow } from '../types';
...
type ErrorsEditorProps = {
  section: ErrorsSection;
  sections: ParsedSection[];
  openInternalCodeKey: string | null;
  setOpenInternalCodeKey: (value: string | null | ((current: string | null) => string | null)) => void;
  highlightedInternalCodeIndex: number;
  setHighlightedInternalCodeIndex: (value: number | ((current: number) => number)) => void;
  internalCodePopoverState: InternalCodePopoverState | null;
  internalCodeAnchorRefs: MutableRefObject<Record<string, HTMLDivElement | null>>;
  internalCodePopoverRef: MutableRefObject<HTMLDivElement | null>;
  validationCaseOptions: string[];
  ...
  updateErrorRow: (sectionId: string, index: number, updater: (current: ErrorRow) => ErrorRow) => void;
  insertJsonResponse: (sectionId: string, index: number) => void;
  applyInternalCode: (sectionId: string, index: number, value: string) => void;
  deleteErrorRow: (sectionId: string, index: number) => void;
  addErrorRow: (sectionId: string) => void;
  updateValidationRuleRow: (sectionId: string, index: number, updater: (current: ValidationRuleRow) => ValidationRuleRow) => void;
  deleteValidationRuleRow: (sectionId: string, index: number) => void;
  addValidationRuleRow: (sectionId: string) => void;
};

export function ErrorsEditor({
  section,
  sections,
  openInternalCodeKey,
            ...
            {section.rows.map((row, index) => {
              ...
              return (
                <tr key={`${section.id}-error-${index}`}>
                  ...
                  <td>
                    <input type="text" disabled value={row.serverHttpStatus} title="Поле заполняется автоматически по internalCode" />
                  </td>
                  <td>
                    <div
                  ...
                  <td>
                    <input type="text" disabled value={row.message} title="Поле заполняется автоматически по internalCode" />
                  </td>
                  <td>
